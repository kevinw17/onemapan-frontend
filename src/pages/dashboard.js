...
export default function Dashboard() {
    ...
    const { data: stats, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                ...
                setEvents(
                    Array.isArray(eventsResponse.data) ? eventsResponse.data.map((event) => ({
                        id: event.event_id,
            ...
            // Hitung total biarawan/biarawati dari dianchuanshi dan spiritualuser dengan is_fuwuyuan true, difilter berdasarkan area
            ...
            if (selectedArea === "Nasional") {
                ...
                activeUsersByMonth = Object.fromEntries(Object.entries(activeUsersByMonth).map(([key, value]) => [key, Math.round(value)]));
            }

            let totalActiveUsersPerArea = 0;
            let activeUsersByMonthPerArea = {};
            if (selectedArea !== "Nasional") {
                totalActiveUsersPerArea = Math.round(filteredUsers.length);
                const oneYearAgo = new Date();
                oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
                ...
                activeUsersByMonthPerArea = Object.fromEntries(Object.entries(activeUsersByMonthPerArea).map(([key, value]) => [key, Math.round(value)]));
            }
            ...
            const userUmatByGender = filteredUsers.length > 0 ? filteredUsers.reduce((acc, u) => {
                ...
                return acc;
            }, []).map(item => ({ ...item, value: Math.round(item.value) })) : [{ gender: "Pria", value: 0 }, { gender: "Wanita", value: 0 }];
    ...
    const lineData = {
        ...
        datasets: [
            {
                label: "Umat Aktif",
                data: Object.values(stats?.activeUsersByMonth || {}).map(value => Math.round(value)),
                fill: false,
                borderColor: "#216ceeff",
                tension: 0.1,
            },
    ...
    const lineDataPerArea = {
        ...
        datasets: [
            {
                label: "Umat Aktif",
                data: Object.values(stats?.activeUsersByMonthPerArea || {}).map(value => Math.round(value)),
                fill: false,
                borderColor: "#216ceeff",
                tension: 0.1,
            },
                        ...
                        <>
                            ...
                            <Box bg="gray.100" borderRadius={16} p={4} display="flex" flexDirection="column" alignItems="center">
                                ...
                                {stats?.totalActiveUsersPerArea > 0 && Object.keys(stats?.activeUsersByMonthPerArea).length > 0 ? (
                                    <>
                                        <Stat textAlign="center" mb={4}>
                                            <StatNumber>{Math.round(stats?.totalActiveUsersPerArea || 0)}</StatNumber>
                                        </Stat>
                                        <Box width="100%" height="250px">
